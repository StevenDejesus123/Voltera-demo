...
ALLOWED_FORMATS = {"csv", "excel", "geojson", "kml", "kmz", "shapefile"}
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    p.add_argument(
        "-f",
        "--format",
        help="Output format: csv|excel|geojson|kml|kmz|shapefile",
    )
    ...
    if not fmt and not args.headless:
        fmt = interactive_choice("Output format (csv|excel|geojson|kml|kmz|shapefile):", "csv")
    if not fmt:
    ...
    opts: Dict = {
        ...
        "regions": regions,
        "format": fmt,
        "output_dir": str(outdir) if outdir else "",
        "ranking_file": ranking_file,
        "headless": bool(args.headless),
    }

    logger.info("Starting export: geography=%s format=%s regions=%s output=%s", geography, fmt, ("ALL" if regions is None else regions), outdir or "(config default)")
